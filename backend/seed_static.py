...
"sql": [
  {
    ...
    'theory': "`SELECT` recupera dados de uma tabela — é o comando mais usado em SQL.\n\n## Como funciona\n\n```sql\nSELECT * FROM produtos;\n```\n\n## No exercício\n\nEscreva a query completa que seleciona todos os dados da tabela `usuarios`.",
    ...
    'quiz': [
      {
        'question': 'Qual cláusula SQL recupera dados de uma tabela?',
        'options': ['GET', 'FETCH', 'SELECT', 'READ'],
        'correct': 2,
      },
      {
        'question': 'O que SELECT * faz em SQL?',
        'options': ['Seleciona a primeira coluna', 'Seleciona todas as colunas', 'Multiplica os valores', 'Seleciona a última linha'],
        'correct': 1,
      },
    ],
  },
  {
    'order': 2,
    'slug': 'sql-select-colunas',
    'title': 'Selecionando colunas específicas',
    'chapter': 'Capítulo 1: Consultando Dados',
    'theory': 'Em vez de `SELECT *`, você pode escolher exatamente quais colunas retornar — mais eficiente e claro.\n\n## Como funciona\n\n```sql\nSELECT nome, preco FROM produtos;\n```\n\n## No exercício\n\nEscreva a query que retorna apenas as colunas `nome` e `email` da tabela `usuarios`.',
    'instruction_pt': 'Liste as colunas desejadas separadas por vírgula depois de `SELECT`. Escreva a query que retorna apenas as colunas `nome` e `email` da tabela `usuarios`.\n\nVeja um exemplo:\n\n```sql\nSELECT nome, preco FROM produtos;\n```',
    'instruction_en': 'List the desired columns separated by commas after `SELECT`. Write the query that returns only `nome` and `email` from the `usuarios` table.',
    'instruction_es': 'Lista las columnas deseadas separadas por coma después de `SELECT`. Escribe la consulta que devuelve solo `nome` y `email` de la tabla `usuarios`.',
    'starter_code': '-- Retorne apenas nome e email da tabela usuarios\n-- escreva aqui\n',
    'hint': 'SELECT coluna1, coluna2 FROM tabela;',
    'hints': [
      'Liste as colunas separadas por vírgula: `SELECT nome, email FROM ...`',
      'Solução: `SELECT nome, email FROM usuarios;`',
      'Selecionar colunas específicas é mais eficiente que `SELECT *` em tabelas grandes.',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'SELECT nome, email FROM usuarios;'}],
    'quiz': [
      {
        'question': 'Como selecionar apenas as colunas nome e idade de uma tabela?',
        'options': [
          'SELECT * FROM tabela WHERE coluna IN (nome, idade)',
          'SELECT nome, idade FROM tabela',
          'SELECT (nome, idade) FROM tabela',
          'GET nome, idade FROM tabela',
        ],
        'correct': 1,
      },
      {
        'question': 'Qual a vantagem de selecionar colunas específicas em vez de SELECT *?',
        'options': [
          'Nenhuma — SELECT * é sempre melhor',
          'Retorna menos dados e melhora a performance',
          'Permite usar WHERE',
          'É obrigatório em bancos grandes',
        ],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'chapter': 'Capítulo 1: Consultando Dados',
    'theory': '`WHERE` filtra as linhas retornadas pela query com base em uma condição.\n\n## Como funciona\n\n```sql\nSELECT * FROM produtos WHERE preco > 50;\n```\n\n## No exercício\n\nEscreva a query que seleciona todos os usuários cuja idade é maior que 18.',
    'instruction_pt': '`WHERE` filtra as linhas retornadas. Operadores comuns: `=`, `>`, `<`, `>=`, `<=`, `!=`. Escreva a query que seleciona todos os usuários cuja idade é maior que 18.\n\nVeja um exemplo:\n\n```sql\nSELECT * FROM produtos WHERE preco > 50;\n```',
    ...
    'hints': [
      '`WHERE` vem depois do `FROM tabela` — use operadores de comparação como `>`, `<`, `=`.',
      'Solução: `SELECT * FROM usuarios WHERE idade > 18;`',
      'Em SQL, igualdade usa `=` (não `==` como em Python/JS).',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'SELECT * FROM usuarios WHERE idade > 18;'}],
    'quiz': [
      {
        'question': 'Qual cláusula SQL filtra linhas por condição?',
        'options': ['FILTER', 'HAVING', 'WHERE', 'WHEN'],
        'correct': 2,
      },
      {
        'question': 'Qual operador verifica igualdade no WHERE em SQL?',
        'options': ['==', ':=', '=', '==='],
        'correct': 2,
      },
      {
        'question': "Como filtrar nomes que começam com 'A' em SQL?",
        'options': ["WHERE nome LIKE 'A%'", "WHERE nome START 'A'", "WHERE nome BEGINS 'A'", "WHERE nome = 'A*'"],
        'correct': 0,
      },
    ],
  },
  {
    'order': 4,
    'slug': 'sql-where-and-or',
    'title': 'WHERE com AND e OR',
    'chapter': 'Capítulo 1: Consultando Dados',
    'theory': '`AND` exige que **todas** as condições sejam verdadeiras; `OR` basta **uma** ser verdadeira. São usados para combinar condições no `WHERE`.\n\n## Como funciona\n\n```sql\nSELECT * FROM produtos WHERE categoria = \'eletronico\' OR preco < 20;\n```\n\n## No exercício\n\nEscreva a query que seleciona usuários de `\'São Paulo\'` com idade maior que 25.',
    'instruction_pt': 'Combine condições no `WHERE` com `AND` (todas verdadeiras) ou `OR` (pelo menos uma). Escreva a query que seleciona usuários da cidade `\'São Paulo\'` e com idade maior que 25.\n\nVeja um exemplo:\n\n```sql\nSELECT * FROM produtos WHERE categoria = \'eletronico\' OR preco < 20;\n```',
    'instruction_en': 'Combine conditions in `WHERE` with `AND` (all true) or `OR` (at least one). Write the query that selects users from `\'São Paulo\'` with age greater than 25.',
    'instruction_es': 'Combina condiciones en `WHERE` con `AND` (todas verdaderas) u `OR` (al menos una). Escribe la consulta que selecciona usuarios de `\'São Paulo\'` con edad mayor a 25.',
    'starter_code': "-- Selecione usuários de 'São Paulo' com idade > 25\n-- escreva aqui\n",
    'hint': "SELECT * FROM usuarios WHERE cidade = 'São Paulo' AND idade > 25;",
    'hints': [
      '`AND` combina condições: as duas precisam ser verdadeiras para a linha aparecer.',
      "Solução: `SELECT * FROM usuarios WHERE cidade = 'São Paulo' AND idade > 25;`",
      'Use parênteses para misturar AND e OR: `WHERE (a OR b) AND c`.',
    ],
    'tests': [{'stdin': '', 'expected_stdout': "SELECT * FROM usuarios WHERE cidade = 'São Paulo' AND idade > 25;"}],
    'quiz': [
      {
        'question': 'Qual operador exige que TODAS as condições sejam verdadeiras?',
        'options': ['OR', 'NOT', 'AND', 'ALL'],
        'correct': 2,
      },
      {
        'question': 'SELECT * FROM t WHERE a = 1 OR b = 2 retorna:',
        'options': [
          'Somente linhas onde a=1 e b=2 ao mesmo tempo',
          'Linhas onde a=1 ou b=2 (ou ambos)',
          'Nenhuma linha',
          'Erro de sintaxe',
        ],
        'correct': 1,
  ...
  },
  {
    ...
    'title': 'ORDER BY',
    'chapter': 'Capítulo 1: Consultando Dados',
    'theory': '`ORDER BY` ordena os resultados — use `ASC` (crescente, padrão) ou `DESC` (decrescente).\n\n## Como funciona\n\n```sql\nSELECT * FROM produtos ORDER BY preco DESC;\n```\n\n## No exercício\n\nEscreva a query que lista todos os usuários ordenados por nome em ordem crescente.',
    'instruction_pt': '`ORDER BY` ordena os resultados. Use `ASC` (crescente, padrão) ou `DESC` (decrescente). Escreva a query que lista todos os usuários ordenados por nome em ordem crescente.\n\nVeja um exemplo:\n\n```sql\nSELECT * FROM produtos ORDER BY preco DESC;\n```',
    ...
    'quiz': [
      {
        'question': 'Qual cláusula SQL ordena os resultados?',
        'options': ['SORT BY', 'ORDER BY', 'GROUP BY', 'ARRANGE BY'],
        'correct': 1,
      },
      {
        'question': 'Para ordenar em ordem decrescente usa-se:',
  ...
  },
  {
    ...
    'title': 'LIMIT',
    'chapter': 'Capítulo 1: Consultando Dados',
    'theory': '`LIMIT` restringe o número de linhas retornadas — muito útil para paginação e para evitar resultados enormes.\n\n## Como funciona\n\n```sql\nSELECT * FROM clientes LIMIT 10;\n```\n\n## No exercício\n\nEscreva a query que retorna os 3 primeiros produtos da tabela `produtos`.',
    'instruction_pt': '`LIMIT` restringe o número de linhas retornadas. Escreva a query que retorna os 3 primeiros produtos da tabela `produtos`.\n\nVeja um exemplo:\n\n```sql\nSELECT * FROM clientes LIMIT 10;\n```',
    ...
    'quiz': [
      {
        'question': 'Qual cláusula SQL limita o número de linhas retornadas?',
        'options': ['MAXIMUM', 'TOP', 'LIMIT', 'ROWS'],
        'correct': 2,
      },
      {
        'question': 'LIMIT é especialmente útil para:',
        'options': ['Filtrar por data', 'Paginação de resultados', 'Ordenar dados', 'Contar registros'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'theory': "`INSERT INTO` adiciona novos registros a uma tabela.\n\n## Como funciona\n\n```sql\nINSERT INTO produtos (nome, preco) VALUES ('Caderno', 15);\n```\n\n## No exercício\n\nEscreva o comando que insere um usuário com nome `'Maria'` e email `'maria@email.com'` na tabela `usuarios`.",
    ...
    'quiz': [
      {
        'question': 'Qual comando SQL adiciona novos registros?',
        'options': ['ADD INTO', 'INSERT INTO', 'PUT INTO', 'CREATE INTO'],
        'correct': 1,
      },
      {
        'question': 'Na sintaxe INSERT INTO, VALUES contém:',
        'options': ['As colunas da tabela', 'Os nomes das tabelas', 'Os valores a inserir', 'Os filtros WHERE'],
        'correct': 2,
      },
    ],
  },
  {
    ...
    'chapter': 'Capítulo 2: Manipulando Dados',
    'theory': '`UPDATE ... SET ... WHERE` atualiza registros existentes — sempre use `WHERE` para evitar alterar todos os registros.\n\n## Como funciona\n\n```sql\nUPDATE produtos SET preco = 20 WHERE id = 3;\n```\n\n## No exercício\n\nEscreva o comando que atualiza o email do usuário com `id = 1` para `\'novo@email.com\'`.',
    ...
    'quiz': [
      {
        'question': 'Qual cláusula SQL define qual linha atualizar em UPDATE?',
        'options': ['WHEN', 'FILTER', 'WHERE', 'SET'],
        'correct': 2,
      },
      {
        'question': 'O que acontece se UPDATE não usar WHERE?',
        'options': ['Nada', 'Atualiza apenas a primeira linha', 'Atualiza todos os registros', 'Gera erro'],
        'correct': 2,
      },
    ],
  },
  {
    ...
    'title': 'DELETE',
    'chapter': 'Capítulo 2: Manipulando Dados',
    'theory': '`DELETE FROM` remove registros — sempre use `WHERE` para evitar apagar tudo!\n\n## Como funciona\n\n```sql\nDELETE FROM produtos WHERE id = 10;\n```\n\n## No exercício\n\nEscreva o comando que remove o usuário com `id = 5`.',
    'instruction_pt': '`DELETE FROM` remove registros. **Sempre use `WHERE`** para evitar apagar tudo! Escreva o comando que remove o usuário com `id = 5`.\n\nVeja um exemplo:\n\n```sql\nDELETE FROM produtos WHERE id = 10;\n```',
    ...
    'quiz': [
      {
        ...
        'options': ['REMOVE FROM', 'DROP FROM', 'DELETE FROM', 'ERASE FROM'],
        'correct': 2,
      },
      {
        'question': 'Por que sempre usar WHERE com DELETE?',
        'options': ['Por convenção', 'Para evitar remover todos os registros', 'Por performance', 'É obrigatório'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'chapter': 'Capítulo 3: Analisando Dados',
    'theory': 'Funções de agregação calculam valores sobre múltiplas linhas: `COUNT`, `SUM`, `AVG`, `MAX`, `MIN`.\n\n## Como funciona\n\n```sql\nSELECT AVG(preco) FROM produtos;\n```\n\n## No exercício\n\nEscreva a query que conta o total de usuários na tabela `usuarios`.',
    ...
    'quiz': [
      {
        'question': 'Qual função SQL conta o total de linhas?',
        'options': ['SUM(*)', 'COUNT(*)', 'TOTAL(*)', 'NUM(*)'],
        'correct': 1,
      },
      {
        'question': 'Qual função SQL calcula a média de uma coluna?',
        'options': ['MEAN()', 'AVERAGE()', 'AVG()', 'MED()'],
        'correct': 2,
      },
      {
        'question': 'Qual função SQL retorna o maior valor de uma coluna?',
        'options': ['GREATEST()', 'MAX()', 'TOP()', 'HIGH()'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'GROUP BY',
    'chapter': 'Capítulo 3: Analisando Dados',
    'theory': '`GROUP BY` agrupa linhas com o mesmo valor em uma coluna — sempre usado com funções de agregação.\n\n## Como funciona\n\n```sql\nSELECT departamento, COUNT(*) FROM funcionarios GROUP BY departamento;\n```\n\n## No exercício\n\nEscreva a query que mostra quantos usuários existem em cada cidade.',
    'instruction_pt': '`GROUP BY` agrupa linhas com o mesmo valor. Sempre usado com funções de agregação. Escreva a query que mostra quantos usuários existem em cada cidade.\n\nVeja um exemplo:\n\n```sql\nSELECT departamento, COUNT(*) FROM funcionarios GROUP BY departamento;\n```',
    ...
    'hints': [
      ...
      'A coluna no SELECT (cidade) deve ser a mesma no GROUP BY.',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'SELECT cidade, COUNT(*) FROM usuarios GROUP BY cidade;'}],
    'quiz': [
      {
        'question': 'GROUP BY agrupa linhas que possuem:',
        'options': ['Valores iguais em uma coluna', 'Datas iguais', 'Qualquer condição', 'Linhas consecutivas'],
        'correct': 0,
      },
      {
        'question': 'GROUP BY sempre é usado junto com:',
        'options': ['WHERE', 'LIMIT', 'Funções de agregação', 'ORDER BY'],
        'correct': 2,
      },
    ],
  },
  {
    ...
    'title': 'HAVING',
    'chapter': 'Capítulo 3: Analisando Dados',
    'theory': '`HAVING` filtra grupos **após** o `GROUP BY` — diferente do `WHERE`, que filtra linhas antes do agrupamento.\n\n## Como funciona\n\n```sql\nSELECT departamento, COUNT(*)\nFROM funcionarios\nGROUP BY departamento\nHAVING COUNT(*) > 3;\n```\n\n## No exercício\n\nEscreva a query que mostra apenas as cidades com mais de 5 usuários.',
    'instruction_pt': '`HAVING` filtra grupos gerados pelo `GROUP BY`. `WHERE` filtra linhas individuais; `HAVING` filtra o resultado agrupado. Escreva a query que mostra apenas as cidades com mais de 5 usuários.\n\nVeja um exemplo:\n\n```sql\nSELECT departamento, COUNT(*)\nFROM funcionarios\nGROUP BY departamento\nHAVING COUNT(*) > 3;\n```',
    'instruction_en': '`HAVING` filters groups after `GROUP BY`. Write the query that shows only cities with more than 5 users.',
    'instruction_es': '`HAVING` filtra grupos después del `GROUP BY`. Escribe la consulta que muestra solo las ciudades con más de 5 usuarios.',
    'starter_code': '-- Mostre apenas as cidades com mais de 5 usuários\n-- escreva aqui\n',
    'hint': 'SELECT cidade, COUNT(*) FROM usuarios GROUP BY cidade HAVING COUNT(*) > 5;',
    'hints': [
      '`HAVING` vem depois do `GROUP BY` — filtra sobre o resultado agrupado.',
      'Solução: `SELECT cidade, COUNT(*) FROM usuarios GROUP BY cidade HAVING COUNT(*) > 5;`',
      'Regra prática: use `WHERE` para filtrar linhas, `HAVING` para filtrar grupos.',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'SELECT cidade, COUNT(*) FROM usuarios GROUP BY cidade HAVING COUNT(*) > 5;'}],
    'quiz': [
      {
        'question': 'Qual a diferença entre WHERE e HAVING?',
        'options': [
          'WHERE filtra grupos; HAVING filtra linhas',
          'WHERE filtra linhas individuais; HAVING filtra grupos após GROUP BY',
          'São sinônimos',
          'HAVING é usado sem GROUP BY',
        ],
        'correct': 1,
      },
      {
        'question': 'HAVING COUNT(*) > 10 retorna:',
        'options': [
          'Linhas onde COUNT > 10',
          'Grupos com mais de 10 registros',
          'Os 10 primeiros grupos',
          'Erro — COUNT não pode ser usado em HAVING',
        ],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'theory': '`JOIN` combina linhas de duas tabelas com base em uma coluna relacionada — `INNER JOIN` retorna apenas as linhas com correspondência em ambas.\n\n## Como funciona\n\n```sql\nSELECT p.nome, c.cidade\nFROM pessoas p\nINNER JOIN cidades c ON p.cidade_id = c.id;\n```\n\n## No exercício\n\nUna as tabelas `usuarios` e `pedidos` pelo id do usuário, selecionando `u.nome` e `p.produto`.',
    'instruction_pt': '`INNER JOIN` combina linhas de duas tabelas com base em uma coluna relacionada, retornando apenas as linhas com correspondência em ambas. Una as tabelas `usuarios` e `pedidos` pelo id do usuário, selecionando `u.nome` e `p.produto`.\n\nVeja um exemplo:\n\n```sql\nSELECT p.nome, c.cidade\nFROM pessoas p\nINNER JOIN cidades c ON p.cidade_id = c.id;\n```',
    ...
    'hints': [
      ...
      'Solução: `SELECT u.nome, p.produto FROM usuarios u INNER JOIN pedidos p ON u.id = p.usuario_id;`',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'SELECT u.nome, p.produto FROM usuarios u INNER JOIN pedidos p ON u.id = p.usuario_id;'}],
    'quiz': [
      {
        'question': 'O que INNER JOIN retorna?',
        'options': [
          'Todos os registros da primeira tabela',
          'Todos os registros de ambas as tabelas',
          'Apenas os registros com correspondência em ambas',
          'Registros sem correspondência',
        ],
        'correct': 2,
      },
      {
        'question': 'Qual cláusula define a condição de ligação no JOIN?',
        'options': ['WHERE', 'ON', 'AND', 'USING'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'CREATE TABLE',
    'chapter': 'Capítulo 4: Relacionamentos',
    'theory': '`CREATE TABLE` cria uma nova tabela. Cada coluna recebe um nome e um tipo (`INT`, `VARCHAR(n)`, `DECIMAL(10,2)`...). `PRIMARY KEY` identifica unicamente cada registro.\n\n## Como funciona\n\n```sql\nCREATE TABLE clientes (\n  id INT PRIMARY KEY,\n  nome VARCHAR(50)\n);\n```\n\n## No exercício\n\nCrie a tabela `produtos` com `id` (INT PRIMARY KEY), `nome` (VARCHAR 100) e `preco` (DECIMAL 10,2).',
    'instruction_pt': '`CREATE TABLE` cria uma nova tabela. Tipos mais comuns:\n- `INT` → inteiro\n- `VARCHAR(n)` → texto até n caracteres\n- `DECIMAL(10,2)` → número decimal\n- `PRIMARY KEY` → chave primária (identificador único)\n\nCrie a tabela `produtos` com `id` (INT PRIMARY KEY), `nome` (VARCHAR 100) e `preco` (DECIMAL 10,2).\n\nVeja um exemplo:\n\n```sql\nCREATE TABLE clientes (\n  id INT PRIMARY KEY,\n  nome VARCHAR(50)\n);\n```',
    'instruction_en': '`CREATE TABLE` creates a new table. Define columns with name and type. `PRIMARY KEY` uniquely identifies each record.',
    'instruction_es': '`CREATE TABLE` crea una nueva tabla. `PRIMARY KEY` identifica de forma única cada registro.',
    'starter_code': '-- Crie a tabela produtos com id (INT PRIMARY KEY), nome (VARCHAR(100)) e preco (DECIMAL(10,2))\n-- escreva aqui\n',
    'hint': 'CREATE TABLE produtos (id INT PRIMARY KEY, nome VARCHAR(100), preco DECIMAL(10,2));',
    'hints': [
      'Estrutura: `CREATE TABLE nome (coluna1 TIPO, coluna2 TIPO, ...);`',
      'Solução: `CREATE TABLE produtos (id INT PRIMARY KEY, nome VARCHAR(100), preco DECIMAL(10,2));`',
      '`PRIMARY KEY` garante que cada registro tem um `id` único — sem duplicatas.',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'CREATE TABLE produtos (id INT PRIMARY KEY, nome VARCHAR(100), preco DECIMAL(10,2));'}],
    'quiz': [
      {
        'question': 'Qual comando SQL cria uma nova tabela?',
        'options': ['NEW TABLE', 'ADD TABLE', 'CREATE TABLE', 'MAKE TABLE'],
        'correct': 2,
      },
      {
        'question': 'O que PRIMARY KEY garante em uma tabela?',
        'options': [
          'Que a coluna aceita apenas números',
          'Que cada registro tem um valor único nessa coluna',
          'Que a coluna é a primeira da tabela',
          'Que a coluna é obrigatória',
        ],
        'correct': 1,
      },
      {
        'question': 'Qual tipo SQL armazena texto de tamanho variável?',
        'options': ['TEXT()', 'CHAR', 'VARCHAR(n)', 'STRING'],
        'correct': 2,
      },
    ],
  },
...
"typescript": [
  {
    ...
    'theory': 'TypeScript adiciona tipagem estática ao JavaScript — você declara o tipo de cada variável e o compilador detecta erros antes de rodar. Os tipos primitivos mais comuns são `string` (texto) e `number` (número).\n\n## Como funciona\n\n```typescript\nlet pontos: number = 10;\n```\n\n## No exercício\n\nEscreva a declaração de uma variável `nome` do tipo `string` sem valor inicial.',
    ...
    'quiz': [
      {
        ...
        'options': ['Mais velocidade de execução', 'Tipagem estática', 'CSS-in-JS', 'Banco de dados embutido'],
        'correct': 1,
      },
      {
        'question': 'Como declarar uma variável string em TypeScript?',
        'options': ['let nome = string', 'string nome;', 'let nome: string;', 'var nome as string;'],
        'correct': 2,
      },
    ],
  },
  {
    'order': 2,
    'slug': 'ts-boolean-void',
    'title': 'boolean e void',
    'chapter': 'Capítulo 1: Tipagem Estática',
    'theory': '`boolean` representa verdadeiro ou falso — é o terceiro tipo primitivo fundamental do TypeScript. `void` é o tipo de retorno de funções que não retornam nenhum valor.\n\n## Como funciona\n\n```typescript\nlet ativo: boolean = true;\n```\n\n## No exercício\n\nDeclare a variável `logado` do tipo `boolean` com valor `false`.',
    'instruction_pt': '`boolean` aceita apenas `true` ou `false`. É amplamente usado em condicionais e flags. Declare a variável `logado` do tipo `boolean` com valor `false`.\n\nVeja um exemplo:\n\n```typescript\nlet ativo: boolean = true;\n```',
    'instruction_en': '`boolean` accepts only `true` or `false`. Declare the variable `logado` of type `boolean` with value `false`.',
    'instruction_es': '`boolean` acepta solo `true` o `false`. Declara la variable `logado` de tipo `boolean` con valor `false`.',
    'starter_code': '// Declare a variável logado do tipo boolean com valor false\n// escreva aqui\n',
    'hint': 'let logado: boolean = false;',
    'hints': [
      '`boolean` aceita apenas `true` ou `false` — é um tipo primitivo do TypeScript.',
      'Solução: `let logado: boolean = false;`',
      'Funções que não retornam nada usam o tipo `void`: `function exibir(): void {`',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'let logado: boolean = false;'}],
    'quiz': [
      {
        'question': 'Qual tipo TypeScript representa verdadeiro ou falso?',
        'options': ['bit', 'bool', 'boolean', 'flag'],
        'correct': 2,
      },
      {
        'question': 'Qual tipo de retorno usar em uma função TypeScript que não retorna nada?',
        'options': ['null', 'undefined', 'void', 'empty'],
        'correct': 2,
      },
      {
        'question': 'Quais são os três tipos primitivos principais do TypeScript?',
        'options': [
          'int, float, char',
  ...
  {
    'order': 3,
    'slug': 'ts-funcao-tipada',
    'title': 'Função tipada',
    'chapter': 'Capítulo 1: Tipagem Estática',
    'theory': 'Em TypeScript, funções têm tipos nos parâmetros e no retorno — isso garante que você não passe o tipo errado por engano.\n\n## Como funciona\n\n```typescript\nfunction multiplicar(a: number, b: number): number {\n```\n\n## No exercício\n\nEscreva a assinatura de uma função `somar` que recebe dois `number` e retorna `number`.',
    ...
    'quiz': [
      {
        'question': 'Como especificar o tipo de retorno de uma função TypeScript?',
        'options': ['function f() as number', 'function f(): number', 'function f() -> number', 'function f() => number'],
        'correct': 1,
      },
      {
        'question': 'Uma vantagem de tipar funções em TypeScript é:',
        'options': ['Código mais lento', 'Evitar erros de tipo em tempo de compilação', 'Menos linhas de código', 'Compatibilidade com Python'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'slug': 'ts-interface',
    'title': 'Interface',
    'chapter': 'Capítulo 2: Tipos Compostos',
    'theory': 'Interfaces definem a "forma" esperada de um objeto — quais campos ele deve ter e de quais tipos.\n\n## Como funciona\n\n```typescript\ninterface Produto { nome: string; preco: number; }\n```\n\n## No exercício\n\nEscreva a interface `Aluno` com os campos `nome` (string) e `idade` (number) em uma linha.',
    ...
    'quiz': [
      {
        'question': 'Qual palavra-chave define uma interface em TypeScript?',
        'options': ['type', 'interface', 'struct', 'schema'],
        'correct': 1,
      },
      {
        'question': 'Interfaces em TypeScript servem para:',
        'options': ['Executar funções', 'Definir a forma esperada de objetos', 'Criar loops', 'Declarar variáveis globais'],
        'correct': 1,
      },
      {
        'question': 'Uma interface pode ter diferentes tipos nas propriedades?',
        'options': ['Não', 'Sim, cada propriedade tem seu próprio tipo', 'Apenas tipos primitivos', 'Apenas string e number'],
        'correct': 1,
      },
    ],
  },
  {
    'order': 5,
    'slug': 'ts-propriedade-opcional',
    'title': 'Propriedade Opcional',
    'chapter': 'Capítulo 2: Tipos Compostos',
    'theory': 'Em interfaces, `?` após o nome do campo torna-o opcional — o objeto pode ou não incluir esse campo.\n\n## Como funciona\n\n```typescript\ninterface Usuario { apelido?: string; }\n```\n\n## No exercício\n\nEscreva a interface `Config` com a propriedade opcional `debug` do tipo `boolean`.',
    ...
    'quiz': [
      {
        'question': 'Como marcar uma propriedade como opcional em interface TypeScript?',
        'options': ['Colocar ! após o nome', 'Colocar ? após o nome', 'Usar o tipo optional', 'Adicionar | undefined'],
        'correct': 1,
      },
      {
        'question': 'interface Config { debug?: boolean } significa:',
        'options': ['debug é obrigatório boolean', 'debug pode estar ausente ou ser boolean', 'debug é sempre true', 'debug aceita qualquer tipo'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'Type Alias',
    'chapter': 'Capítulo 2: Tipos Compostos',
    'theory': "`type` cria um apelido reutilizável para um tipo — muito usado com union types para representar opções fixas.\n\n## Como funciona\n\n```typescript\ntype Tamanho = 'pequeno' | 'medio' | 'grande';\n```\n\n## No exercício\n\nEscreva o type alias `Cor` que aceita os valores literais `'vermelho'`, `'azul'` ou `'verde'`.",
    ...
    'hint': "type NomeAlias = 'valor1' | 'valor2' | 'valor3';",
    'hints': [
    ...
    'quiz': [
      {
        'question': 'O que um type alias faz em TypeScript?',
        'options': ['Renomeia variáveis', 'Cria um apelido reutilizável para um tipo', 'Define uma interface', 'Cria uma classe'],
        'correct': 1,
      },
      {
        'question': 'Qual símbolo separa valores em union types dentro de um type alias?',
        'options': ['&', '|', '+', '~'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'Union Types',
    'chapter': 'Capítulo 2: Tipos Compostos',
    'theory': 'Union types permitem que uma variável aceite mais de um tipo, combinando com `|`.\n\n## Como funciona\n\n```typescript\nlet codigo: number | string;\n```\n\n## No exercício\n\nDeclare a variável `id` que pode ser `number` ou `string`.',
    ...
    'quiz': [
      {
        'question': 'O que let id: number | string significa?',
        'options': ['id pode ser number e string ao mesmo tempo', 'id pode ser number ou string', 'id é do tipo number_string', 'Erro de sintaxe'],
        'correct': 1,
      },
      {
        'question': 'Union types usam qual operador?',
        'options': ['+', '&', '|', '~'],
        'correct': 2,
      },
    ],
  ...
  {
    'order': 8,
    'slug': 'ts-arrays-tipados',
    'title': 'Arrays Tipados',
    'chapter': 'Capítulo 3: Coleções e Enums',
    'theory': "Em TypeScript, arrays têm tipos: `tipo[]` define que todos os elementos devem ser daquele tipo.\n\n## Como funciona\n\n```typescript\nlet nomes: string[] = ['Ana', 'Bia'];\n```\n\n## No exercício\n\nDeclare a variável `numeros` como um array de `number` com os valores `[1, 2, 3]`.",
    ...
    'quiz': [
      {
        'question': 'Como declarar um array de strings em TypeScript?',
        'options': ['string array', 'Array(string)', 'string[]', 'list<string>'],
        'correct': 2,
      },
      {
        'question': 'O que number[] representa em TypeScript?',
        'options': ['Um número com índice', 'Um array de números', 'Uma matriz 2D', 'Um tipo numérico especial'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'Enum',
    'chapter': 'Capítulo 3: Coleções e Enums',
    'theory': '`enum` define um conjunto nomeado de constantes — útil quando uma variável só pode assumir valores predefinidos.\n\n## Como funciona\n\n```typescript\nenum Status { Ativo, Inativo, Pendente }\n```\n\n## No exercício\n\nEscreva o enum `Direcao` com os membros `Norte`, `Sul`, `Leste` e `Oeste` em uma linha.',
    ...
    'quiz': [
      {
        'question': 'Para que serve enum em TypeScript?',
        'options': ['Criar loops', 'Definir um conjunto de constantes nomeadas', 'Fazer herança', 'Importar módulos'],
        'correct': 1,
      },
      {
        'question': 'Qual é o valor padrão de enum Cor { Vermelho, Azul }?',
        'options': ["'Vermelho', 'Azul'", 'null, null', '0, 1', '1, 2'],
        'correct': 2,
      },
    ],
  },
  {
    ...
    'theory': 'Generics permitem criar funções e classes reutilizáveis para qualquer tipo — sem perder a tipagem.\n\n## Como funciona\n\n```typescript\nfunction primeiro<T>(lista: T[]): T {\n```\n\n## No exercício\n\nEscreva a assinatura da função genérica `identidade` que recebe um parâmetro `valor` do tipo `T` e retorna `T`.',
    'instruction_pt': 'Generics permitem criar funções reutilizáveis para qualquer tipo. Escreva a assinatura da função genérica `identidade` que recebe um parâmetro `valor` do tipo `T` e retorna `T`.\n\nVeja um exemplo:\n\n```typescript\nfunction primeiro<T>(lista: T[]): T {\n```',
    ...
    'quiz': [
      {
        'question': 'O que <T> representa em uma função genérica TypeScript?',
        'options': ['O nome da função', 'Um tipo que será definido na chamada', 'TypeScript interno', 'Um template HTML'],
        'correct': 1,
      },
      {
        'question': 'A vantagem dos generics é:',
        'options': ['Código mais rápido', 'Reutilização tipada para qualquer tipo', 'Menos memória', 'Compilação mais rápida'],
        'correct': 1,
      },
    ],
  },
  {
    'order': 11,
    'slug': 'ts-classe-tipada',
    'title': 'Classe Tipada',
    'chapter': 'Capítulo 4: Recursos Avançados',
    'theory': 'Classes em TypeScript podem ter propriedades tipadas declaradas diretamente no construtor usando `public` — isso cria e atribui a propriedade automaticamente.\n\n## Como funciona\n\n```typescript\nconstructor(public idade: number) {}\n```\n\n## No exercício\n\nEscreva a linha `constructor` da classe `Animal` com a propriedade pública `nome: string`.',
    ...
    'quiz': [
      {
        'question': "O que 'public' faz no constructor de uma classe TypeScript?",
        'options': ['Deixa a classe pública', 'Cria e atribui automaticamente a propriedade', 'Permite herança', 'Define o tipo de retorno'],
        'correct': 1,
      },
      {
        'question': 'Classes TypeScript funcionam de modo similar a:',
        'options': ['Python (def)', 'Java/C++ (class com tipos)', 'Go (struct)', 'Lua (table)'],
        'correct': 1,
      },
    ],
  },
...
"java": [
  {
    'order': 1,
    'slug': 'java-imprimir-console',
    'title': 'Imprimir no console',
    'chapter': 'Capítulo 1: Fundamentos Java',
    'theory': 'Java usa `System.out.println()` para imprimir no console com quebra de linha automática.\n\n## Como funciona\n\n```java\nSystem.out.println("Bem-vindo!");\n```\n\n## No exercício\n\nEscreva apenas o comando que imprime `"Olá, Java!"`.',
    ...
    'quiz': [
      {
        'question': 'Como imprimir texto com quebra de linha em Java?',
        'options': ['System.print()', 'Console.log()', 'System.out.println()', 'print()'],
        'correct': 2,
      },
      {
        'question': 'O que System.out.print() faz diferente de println()?',
        'options': ['Imprime em vermelho', 'Não adiciona quebra de linha no final', 'Imprime em maiúsculas', 'Salva em arquivo'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'chapter': 'Capítulo 1: Fundamentos Java',
    'theory': 'Java é fortemente tipado — toda variável precisa de um tipo declarado. Tipos comuns: `int` (inteiro), `double` (decimal), `boolean` (verdadeiro/falso).\n\n## Como funciona\n\n```java\nint pontos = 100;\n```\n\n## No exercício\n\nEscreva a declaração de um inteiro `idade` com valor 25.',
    'instruction_pt': 'Java é fortemente tipado — toda variável precisa de um tipo. `int` para inteiros, `double` para decimais, `boolean` para verdadeiro/falso. Escreva a declaração de um inteiro `idade` com valor 25.\n\nVeja um exemplo:\n\n```java\nint pontos = 100;\n```',
    ...
    'quiz': [
      {
        'question': 'Como declarar um inteiro em Java?',
        'options': ['integer x = 5;', 'Int x = 5;', 'int x = 5;', 'number x = 5;'],
        'correct': 2,
      },
      {
        'question': 'Java é uma linguagem:',
        'options': ['Dinamicamente tipada', 'Fortemente tipada (requer declaração de tipo)', 'Sem tipos', 'Tipada opcionalmente'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'chapter': 'Capítulo 1: Fundamentos Java',
    'theory': '`String` em Java é uma **classe** (não um tipo primitivo) — sempre com `S` maiúsculo. Strings têm métodos úteis como `.length()`, `.toUpperCase()` e `.contains()`.\n\n## Como funciona\n\n```java\nString saudacao = "Bem-vindo";\n```\n\n## No exercício\n\nDeclare uma `String` chamada `msg` com o valor `"Olá, Java!"`.',
    'instruction_pt': '`String` em Java é uma classe — declare com `String nome = "valor";`. Escreva a declaração da `String` chamada `msg` com o valor `"Olá, Java!"`.\n\nVeja um exemplo:\n\n```java\nString saudacao = "Bem-vindo";\n```',
    'instruction_en': '`String` in Java is a class — declare with `String name = "value";`. Write the declaration of the `String` `msg` with value `"Olá, Java!"`.',
    'instruction_es': '`String` en Java es una clase. Escribe la declaración de la `String` llamada `msg` con el valor `"Olá, Java!"`.',
    'starter_code': '// Declare uma String chamada msg com o valor "Olá, Java!"\n// escreva aqui\n',
    'hint': 'String nomeDaVariavel = "valor";',
    'hints': [
      'Em Java, `String` (com S maiúsculo) é uma classe — declare como: `String nome = "valor";`',
      'Solução: `String msg = "Olá, Java!";`',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'String msg = "Olá, Java!";'}],
    'quiz': [
      {
        'question': 'Em Java, String é:',
        'options': ['Um tipo primitivo como int', 'Uma classe (objeto)', 'Um array de char básico', 'Um tipo numérico'],
        'correct': 1,
      },
      {
        'question': 'Como obter o número de caracteres de uma String em Java?',
        'options': ['str.size()', 'str.count()', 'str.len', 'str.length()'],
        'correct': 3,
      },
      {
        'question': 'Como se escreve String em Java?',
        'options': ['string (minúsculo)', 'STRING', 'String (S maiúsculo)', 'str'],
        'correct': 2,
      },
    ],
  },
  {
    'order': 4,
    'slug': 'java-condicionais',
    'title': 'Condicionais',
    'chapter': 'Capítulo 2: Controle de Fluxo',
    'theory': 'Java usa `if (condição) { ... } else { ... }` para tomar decisões — a condição fica obrigatoriamente entre parênteses.\n\n## Como funciona\n\n```java\nif (temperatura > 30) {\n```\n\n## No exercício\n\nEscreva o cabeçalho do `if` que verifica se a variável `x` é maior que zero.',
    'instruction_pt': 'Java usa `if (condição) { ... }` para tomar decisões. A condição é obrigatoriamente entre parênteses. Escreva o cabeçalho do `if` que verifica se a variável `x` é maior que zero.\n\nVeja um exemplo:\n\n```java\nif (temperatura > 30) {\n```',
    'instruction_en': 'Java uses `if (condition) { ... }` to make decisions. Write the header of the `if` that checks if variable `x` is greater than zero.',
    'instruction_es': 'Java usa `if (condición) { ... }` para tomar decisiones. Escribe el encabezado del `if` que verifica si la variable `x` es mayor que cero.',
    ...
    'quiz': [
      {
        'question': 'Java usa parênteses na condição do if?',
        'options': ['Não, é opcional', 'Sim, são obrigatórios', 'Só em else if', 'Depende da versão'],
        'correct': 1,
      },
      {
        'question': 'Como escrever else-if em Java?',
        'options': ['elseif (cond)', 'elif (cond)', 'else if (cond)', 'otherwise (cond)'],
        'correct': 2,
      },
    ],
  },
  {
    ...
    'title': 'Loop For',
    'chapter': 'Capítulo 2: Controle de Fluxo',
    'theory': 'O `for` em Java tem três partes separadas por `;`: inicialização, condição e incremento.\n\n## Como funciona\n\n```java\nfor (int i = 10; i > 0; i--) {\n```\n\n## No exercício\n\nEscreva o cabeçalho do `for` que conta de 0 até 4 (i < 5).',
    ...
    'quiz': [
      {
        'question': 'Qual é a ordem das partes do for em Java?',
        'options': ['condição; incremento; init', 'init; condição; incremento', 'incremento; init; condição', 'init; incremento; condição'],
        'correct': 1,
      },
      {
        'question': 'O que i++ faz em Java?',
        'options': ['Multiplica por i', 'Incrementa i em 1', 'Decrementa i em 1', 'Reinicia i'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'Loop While',
    'chapter': 'Capítulo 2: Controle de Fluxo',
    'theory': '`while (condição) { ... }` repete o bloco enquanto a condição for verdadeira — ideal quando o número de repetições não é conhecido antecipadamente.\n\n## Como funciona\n\n```java\nwhile (saldo > 0) {\n```\n\n## No exercício\n\nEscreva o cabeçalho do `while` que continua enquanto `tentativas < 3`.',
    'instruction_pt': '`while` repete o bloco enquanto a condição for verdadeira. Use-o quando não sabe quantas vezes o loop vai repetir. Escreva o cabeçalho do `while` que continua enquanto `tentativas < 3`.\n\nVeja um exemplo:\n\n```java\nwhile (saldo > 0) {\n```',
    'instruction_en': '`while` repeats the block while the condition is true. Write the header of the `while` that continues while `tentativas < 3`.',
    'instruction_es': '`while` repite el bloque mientras la condición sea verdadera. Escribe el encabezado del `while` que continúa mientras `tentativas < 3`.',
    'starter_code': '// Escreva o cabeçalho do while que executa enquanto tentativas < 3\n// Dica: while (condicao) {\n// escreva aqui\n',
    'hint': 'while (variavel < valor) {',
    'hints': [
      '`while` tem apenas a condição — sem init ou incremento: `while (condição) {`',
      'Solução: `while (tentativas < 3) {`',
      'Atenção: o incremento da variável deve estar dentro do bloco, senão vira loop infinito!',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'while (tentativas < 3) {'}],
    'quiz': [
      {
        'question': 'Quando usar while em vez de for em Java?',
        'options': [
          'Quando o número exato de repetições é conhecido',
          'Quando o número de repetições depende de uma condição variável',
          'Nunca — for é sempre melhor',
          'Apenas para loops infinitos',
        ],
        'correct': 1,
      },
      {
        'question': 'O que acontece se a condição do while nunca for falsa?',
        'options': ['O programa termina normalmente', 'Loop infinito', 'Erro de compilação', 'O Java reinicia o loop'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'slug': 'java-arrays',
    'title': 'Arrays',
    'chapter': 'Capítulo 3: Estruturas de Dados',
    'theory': 'Arrays em Java têm tamanho fixo e tipo declarado — todos os elementos devem ser do mesmo tipo.\n\n## Como funciona\n\n```java\nint[] notas = {7, 8, 9};\n```\n\n## No exercício\n\nEscreva a declaração do array `numeros` do tipo `int[]` com os valores `{1, 2, 3}` em uma linha.',
    ...
    'quiz': [
      {
        'question': 'Arrays em Java têm tamanho:',
        'options': ['Dinâmico (cresce automaticamente)', 'Fixo (definido na criação)', 'Indefinido', 'Limitado a 100 elementos'],
        'correct': 1,
      },
      {
        'question': 'Como acessar o terceiro elemento de int[] arr em Java?',
        'options': ['arr[3]', 'arr.get(3)', 'arr[2]', 'arr.third()'],
        'correct': 2,
      },
    ],
  },
  {
    ...
    'title': 'ArrayList',
    'chapter': 'Capítulo 3: Estruturas de Dados',
    'theory': '`ArrayList` é uma lista de tamanho **dinâmico** — ao contrário do array fixo, ela cresce conforme você adiciona elementos.\n\n## Como funciona\n\n```java\nArrayList<Integer> notas = new ArrayList<>();\n```\n\n## No exercício\n\nEscreva a declaração de um ArrayList de String chamado `nomes`.',
    ...
    'hints': [
      ...
      "O `<>` é o 'diamond operator' — o tipo é inferido do lado esquerdo.",
      'Solução: `ArrayList<String> nomes = new ArrayList<>();`',
    ],
    'tests': [{'stdin': '', 'expected_stdout': 'ArrayList<String> nomes = new ArrayList<>();'}],
    'quiz': [
      {
        'question': 'Qual é a diferença entre Array e ArrayList em Java?',
        'options': ['Array é mais lento', 'ArrayList tem tamanho dinâmico, Array tem tamanho fixo', 'ArrayList não aceita objetos', 'São idênticos'],
        'correct': 1,
      },
      {
        'question': 'Qual método adiciona elemento ao ArrayList em Java?',
  ...
  {
    'order': 9,
    'slug': 'java-metodo-estatico',
    'title': 'Método estático',
    'chapter': 'Capítulo 4: Orientação a Objetos',
    'theory': 'Métodos em Java são declarados com tipo de retorno, nome e parâmetros. `static` significa que o método pertence à classe — não a uma instância.\n\n## Como funciona\n\n```java\npublic static int triplicar(int n) {\n```\n\n## No exercício\n\nEscreva a assinatura do método `dobrar` que recebe e retorna `int`.',
    'instruction_pt': 'Métodos em Java: `public static TipoRetorno nomeMetodo(Tipo param)`. Escreva a assinatura do método `dobrar` que recebe e retorna `int`.\n\nVeja um exemplo:\n\n```java\npublic static int triplicar(int n) {\n```',
    ...
    'quiz': [
      {
        'question': "O que 'static' significa em um método Java?",
        'options': ['O método não muda', 'O método pertence à classe, não a uma instância', 'O método é privado', 'O método é final'],
        'correct': 1,
      },
      {
        'question': 'Qual é o tipo de retorno void em Java?',
        'options': ['Retorna zero', 'Retorna string vazia', 'Não retorna nada', 'Retorna null sempre'],
        'correct': 2,
      },
    ],
  },
  {
    'order': 10,
    'slug': 'java-classe-construtor',
    'title': 'Classe com Construtor',
    'chapter': 'Capítulo 4: Orientação a Objetos',
    'theory': 'Uma classe em Java agrupa dados e comportamentos. O construtor tem o mesmo nome da classe e inicializa o objeto — não tem tipo de retorno.\n\n## Como funciona\n\n```java\nProduto(String nome) {\n```\n\n## No exercício\n\nEscreva a assinatura do construtor da classe `Pessoa` que recebe o parâmetro `String nome`.',
    'instruction_pt': 'O construtor tem o mesmo nome da classe e não tem tipo de retorno. Escreva a assinatura do construtor da classe `Pessoa` que recebe o parâmetro `String nome`.\n\nVeja um exemplo:\n\n```java\nProduto(String nome) {\n```',
    'instruction_en': 'A class in Java groups data and behavior. Write the constructor signature of the `Pessoa` class that receives `String nome`.',
    ...
    'quiz': [
      {
        'question': 'O construtor de uma classe Java:',
        'options': ['Tem o mesmo nome da classe', 'Sempre retorna void', "É chamado com 'create'", 'É opcional em toda classe'],
        'correct': 0,
      },
      {
        'question': "O que 'this' representa no construtor Java?",
        'options': ['A classe pai', 'A instância sendo criada', 'O método main', 'O pacote atual'],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'Herança',
    'chapter': 'Capítulo 4: Orientação a Objetos',
    'theory': 'Herança em Java usa `extends` — a subclasse herda todos os atributos e métodos da superclasse. Java permite herança de apenas uma classe por vez.\n\n## Como funciona\n\n```java\npublic class Gerente extends Funcionario {\n```\n\n## No exercício\n\nEscreva a declaração da classe `Aluno` que herda de `Pessoa`.',
    'instruction_pt': 'Herança em Java usa `extends`. A subclasse herda todos os métodos e atributos da superclasse. Escreva a declaração da classe `Aluno` que herda de `Pessoa`.\n\nVeja um exemplo:\n\n```java\npublic class Gerente extends Funcionario {\n```',
    ...
    'quiz': [
      {
        'question': 'Qual palavra-chave implementa herança em Java?',
        'options': ['inherits', 'implements', 'extends', ':'],
        'correct': 2,
      },
      {
        'question': 'Uma classe Java pode herdar de quantas classes simultaneamente?',
        'options': ['Quantas quiser', 'Apenas uma', 'Máximo de três', 'Apenas de classes abstratas'],
        'correct': 1,
      },
    ],
  },
...
"cpp": [
  {
    'order': 1,
    'slug': 'cpp-saida-console',
    'title': 'Saída no console',
    'chapter': 'Capítulo 1: Fundamentos C++',
    'theory': 'C++ usa `cout << "texto" << endl;` para exibir texto, com `endl` quebrando a linha.\n\n## Como funciona\n\n```cpp\ncout << "Bem-vindo!" << endl;\n```\n\n## No exercício\n\nEscreva o comando que exibe \'Olá, C++!\'.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'cout << "Olá, C++!" << endl;',
      },
    ],
    'quiz': [
      {
        'question': 'Qual objeto exibe texto no console em C++?',
        'options': [
          'System.out',
          'Console',
          'cout',
      ...
      {
        'question': "O que 'endl' faz em C++?",
        'options': [
          'Finaliza o programa',
          'Quebra a linha (equivalente a \\n)',
          'Limpa o buffer',
          'Define o fim do array',
        ],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'Declarando variáveis',
    'chapter': 'Capítulo 1: Fundamentos C++',
    'theory': 'C++ exige declaração de tipo: `int`, `double`, `string`.\n\n## Como funciona\n\n```cpp\nint pontos = 100;\n```\n\n## No exercício\n\nEscreva a declaração de um inteiro `nota` com valor 10.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'int nota = 10;',
      },
    ],
    'quiz': [
      {
        'question': 'Qual tipo C++ armazena números decimais?',
        'options': [
          'float/double',
          'int',
          'char',
      ...
      {
        'question': 'C++ requer declaração explícita de tipo?',
        'options': [
          'Não, é opcional',
          'Sim, sempre',
          'Só para funções',
  ...
  },
  {
    ...
    'title': 'String em C++',
    'chapter': 'Capítulo 1: Fundamentos C++',
    'theory': 'C++ tem o tipo `string` (da biblioteca `<string>`).\n\n## Como funciona\n\n```cpp\nstring saudacao = "Bem-vindo";\n```\n\n## No exercício\n\nDeclare a variável `nome` do tipo `string` com o valor `\'Carlos\'`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'string nome = "Carlos";',
      },
    ],
    'quiz': [
      {
        'question': 'Para usar string em C++ você precisa incluir:',
        'options': [
          '<stdio.h>',
          '<string>',
          '<text>',
      ...
      {
        'question': 'Qual a diferença entre char e string em C++?',
        'options': [
          'char é um texto, string é um caractere',
          'char é um único caractere, string é uma sequência',
          'São idênticos',
          'char é mais rápido sempre',
        ],
        'correct': 1,
      },
    ],
  ...
  {
    'order': 4,
    'slug': 'cpp-condicionais',
    'title': 'Condicionais',
    'chapter': 'Capítulo 2: Controle de Fluxo',
    'theory': 'C++ usa `if (condição) { ... } else { ... }`.\n\n## Como funciona\n\n```cpp\nif (idade >= 18) {\n```\n\n## No exercício\n\nEscreva o cabeçalho do `if` que verifica se a variável `x` é maior que zero.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'if (x > 0) {',
      },
    ],
    'quiz': [
      {
        'question': 'C++ usa chaves {} para delimitar blocos condicionais?',
        'options': [
          'Não, usa indentação',
          'Sim, definem o início e fim do bloco',
          'Só em funções',
          'Só com else',
        ],
        'correct': 1,
      ...
      {
        'question': "Qual operador lógico significa 'E' em C++?",
        'options': [
          'and',
          '&&',
          '||',
  ...
  },
  {
    ...
    'title': 'Loop For',
    'chapter': 'Capítulo 2: Controle de Fluxo',
    'theory': 'O `for` em C++ funciona igual ao Java: inicialização, condição e incremento.\n\n## Como funciona\n\n```cpp\nfor (int i = 10; i > 0; i--) {\n```\n\n## No exercício\n\nEscreva o cabeçalho do `for` que conta de 0 até 4 (i < 5).',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'for (int i = 0; i < 5; i++) {',
      },
    ],
    'quiz': [
      {
        'question': 'O loop for em C++ é idêntico ao de:',
        'options': [
          'Python',
          'Java/JavaScript',
          'Go',
  ...
  },
  {
    ...
    'title': 'Loop While',
    'chapter': 'Capítulo 2: Controle de Fluxo',
    'theory': 'O `while` executa um bloco enquanto a condição for verdadeira — ideal quando o número de iterações é desconhecido.\n\n## Como funciona\n\n```cpp\nwhile (tentativas > 0) {\n```\n\n## No exercício\n\nEscreva o cabeçalho do `while` que executa enquanto `i` for menor que 10.',
    'instruction_pt': 'O `while` executa enquanto a condição for verdadeira. Escreva o cabeçalho do `while` que executa enquanto `i` for menor que 10.\n\nVeja um exemplo:\n\n```cpp\nwhile (tentativas > 0) {\n```',
    'instruction_en': 'The `while` loop runs while the condition is true. Write the `while` header that runs while `i` is less than 10.',
    'instruction_es': 'El `while` ejecuta mientras la condición sea verdadera. Escribe el encabezado del `while` que se ejecuta mientras `i` sea menor que 10.',
    'starter_code': '// Escreva o cabeçalho do while que executa enquanto i < 10\n// Dica: while (condicao) {\n// escreva aqui\n',
    'hint': 'while (variavel < valor) {',
    'hints': [
      '`while` usa apenas a condição — sem inicialização nem incremento no cabeçalho.',
      'Solução: `while (i < 10) {`',
    ],
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'while (i < 10) {',
      },
    ],
    'quiz': [
      {
        'question': 'Quando preferir while em vez de for?',
        'options': [
          'Sempre',
          'Quando o número de iterações é desconhecido',
          'Quando a condição é complexa',
          'Nunca — for é suficiente',
        ],
        'correct': 1,
      },
      {
        ...
        'options': [
          'O programa encerra normalmente',
          'Ocorre loop infinito',
          'Gera erro de compilação',
          'O bloco executa uma única vez',
        ],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'slug': 'cpp-funcao',
    'title': 'Função',
    'chapter': 'Capítulo 3: Funções e Arrays',
    'theory': 'Funções C++ têm o tipo de retorno antes do nome.\n\n## Como funciona\n\n```cpp\nint multiplicar(int a, int b) {\n```\n\n## No exercício\n\nEscreva a assinatura da função `soma` que recebe dois inteiros e retorna inteiro.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'int soma(int a, int b) {',
      },
    ],
    'quiz': [
      {
        'question': 'Em C++, onde fica o tipo de retorno na declaração de função?',
        'options': [
      ...
      {
        'question': 'O que void como tipo de retorno significa em C++?',
        'options': [
          'Retorna zero',
          'Retorna null',
          'Função sem retorno',
  ...
  },
  {
    ...
    'title': 'Arrays',
    'chapter': 'Capítulo 3: Funções e Arrays',
    'theory': 'Arrays em C++ têm tamanho fixo.\n\n## Como funciona\n\n```cpp\nint notas[] = {7, 8, 9};\n```\n\n## No exercício\n\nDeclare o array de inteiros `numeros` com os valores `{1, 2, 3}` em uma linha.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'int numeros[] = {1, 2, 3};',
      },
    ],
    'quiz': [
      {
        'question': 'Arrays em C++ têm tamanho:',
        'options': [
          'Dinâmico',
          'Fixo definido na criação',
          'Ilimitado',
          'Até 256 elementos',
        ],
        'correct': 1,
  ...
  },
  {
    ...
    'slug': 'cpp-vector',
    'title': 'Vector',
    'chapter': 'Capítulo 4: Tipos Compostos',
    'theory': '`vector` é a coleção dinâmica de C++ (da `<vector>`).\n\n## Como funciona\n\n```cpp\nvector<int> notas = {7, 8, 9};\n```\n\n## No exercício\n\nEscreva a declaração do vector de inteiros `numeros` com os valores `{1, 2, 3}`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'vector<int> numeros = {1, 2, 3};',
      },
    ],
    'quiz': [
      {
        'question': 'Qual é a vantagem do vector sobre arrays em C++?',
        'options': [
          'É mais rápido sempre',
          'Tem tamanho dinâmico (cresce conforme necessário)',
          'Usa menos memória',
          'Não precisa de tipo',
        ],
        'correct': 1,
      ...
      {
        'question': 'Para usar vector em C++ você precisa incluir:',
        'options': [
          '<array>',
          '<list>',
          '<vector>',
  ...
  },
  {
    ...
    'title': 'Struct',
    'chapter': 'Capítulo 4: Tipos Compostos',
    'theory': '`struct` em C++ agrupa variáveis relacionadas.\n\n## Como funciona\n\n```cpp\nstruct Produto { string nome; double preco; };\n```\n\n## No exercício\n\nEscreva a declaração da struct `Pessoa` com os campos `string nome` e `int idade` em uma linha.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'struct Pessoa { string nome; int idade; };',
      },
    ],
    'quiz': [
      {
        'question': 'Para que serve struct em C++?',
        'options': [
          'Criar loops',
      ...
      {
        'question': 'Diferença entre struct e class em C++:',
        'options': [
          'struct tem membros públicos por padrão, class tem privados',
          'São completamente iguais',
          'struct não tem métodos',
          'class não tem dados',
        ],
        'correct': 0,
      },
  ...
  },
  {
    ...
    'title': 'Ponteiros',
    'chapter': 'Capítulo 4: Tipos Compostos',
    'theory': 'Ponteiros armazenam o endereço de memória de uma variável.\n\n## Como funciona\n\n```cpp\nint* contador = &total;\n```\n\n## No exercício\n\nEscreva essa declaração de ponteiro para a variável inteira `x`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'int* ptr = &x;',
      },
    ],
    'quiz': [
      {
        'question': 'O que um ponteiro armazena em C++?',
        'options': [
          'Um valor inteiro',
      ...
      {
        'question': 'O que o operador & faz com uma variável?',
        'options': [
          'Copia a variável',
          'Retorna o endereço de memória',
          'Decrementa o valor',
          'Cria uma referência constante',
        ],
        'correct': 1,
      },
      {
        'question': 'O que o operador * faz com um ponteiro?',
        'options': [
          'Multiplica o endereço',
          'Desreferencia (acessa o valor apontado)',
...
"go": [
  {
    ...
    'theory': 'Go usa `fmt.Println()` para exibir texto com quebra de linha.\n\n## Como funciona\n\n```go\nfmt.Println("Bem-vindo!")\n```\n\n## No exercício\n\nEscreva o comando que exibe \'Olá, Go!\'.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'fmt.Println("Olá, Go!")',
      },
    ],
    'quiz': [
      {
        'question': 'Qual pacote Go precisa ser importado para imprimir?',
        'options': [
          'os',
          'io',
          'fmt',
      ...
      {
        'question': 'Qual função Go imprime com quebra de linha no final?',
        'options': [
          'fmt.Print()',
          'fmt.Println()',
          'fmt.Printf()',
  ...
  {
    'order': 2,
    'slug': 'go-declaracao-curta',
    'title': 'Declaração curta',
    'chapter': 'Capítulo 1: Introdução ao Go',
    'theory': 'Go permite declarar variáveis com `:=` — o compilador infere o tipo automaticamente.\n\n## Como funciona\n\n```go\ncidade := "Curitiba"\n```\n\n## No exercício\n\nEscreva a declaração da variável `nome` com valor \'Gopher\'.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'nome := "Gopher"',
      },
    ],
    'quiz': [
      {
        'question': 'O que := faz em Go?',
        'options': [
          'Compara dois valores',
          'Declara e atribui uma variável com tipo inferido',
      ...
      {
        'question': 'Qual é a diferença entre := e var em Go?',
        'options': [
          ':= só funciona em funções e infere o tipo; var pode ser global',
          'São idênticos',
          'var é mais rápido',
          ':= exige tipo explícito',
        ],
        'correct': 0,
      },
    ],
  },
  {
    ...
    'slug': 'go-funcao',
    'title': 'Função',
    'chapter': 'Capítulo 3: Funções e Coleções',
    'theory': 'Go declara funções com `func nome(params tipo) tipoRetorno`.\n\n## Como funciona\n\n```go\nfunc triplicar(n int) int {\n```\n\n## No exercício\n\nEscreva a assinatura da função `dobrar` que recebe `n int` e retorna `int`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'func dobrar(n int) int {',
      },
    ],
    'quiz': [
      {
        'question': 'Qual palavra-chave define uma função em Go?',
        'options': [
          'def',
          'function',
          'fn',
      ...
      {
        'question': 'Em Go, o tipo de retorno fica:',
        'options': [
          'Antes do nome da função',
          'Após os parâmetros',
          'Entre parênteses',
  ...
  },
  {
    ...
    'title': 'Slices',
    'chapter': 'Capítulo 3: Funções e Coleções',
    'theory': 'Slices são arrays dinâmicos em Go.\n\n## Como funciona\n\n```go\nnotas := []int{7, 8, 9}\n```\n\n## No exercício\n\nDeclare o slice de inteiros `numeros` com os valores `{1, 2, 3}` usando a sintaxe literal.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'numeros := []int{1, 2, 3}',
      },
    ],
    'quiz': [
      {
        'question': 'Qual é a diferença entre slice e array em Go?',
        'options': [
          'Slice tem tipo diferente',
          'Slice tem tamanho dinâmico, array tem tamanho fixo',
          'Array é mais moderno',
          'São idênticos',
        ],
        'correct': 1,
      },
      {
        'question': 'Como declarar um slice literal de ints em Go?',
        'options': [
          'int[]{1, 2, 3}',
          '[]int{1, 2, 3}',
          '(1, 2, 3)',
  ...
  },
  {
    ...
    'title': 'Maps',
    'chapter': 'Capítulo 3: Funções e Coleções',
    'theory': 'Maps são dicionários em Go.\n\n## Como funciona\n\n```go\nprecos := map[string]int{"Caderno": 15}\n```\n\n## No exercício\n\nEscreva a declaração do map `idades` que mapeia `string` para `int`, com um par `\'Ana\': 20` usando sintaxe literal.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'idades := map[string]int{"Ana": 20}',
      },
    ],
    'quiz': [
      {
        'question': 'Qual é o equivalente de dicionário em Go?',
        'options': [
          'dict',
          'hashmap',
          'map',
      ...
      {
        'question': 'Como declarar um map de string para int em Go?',
        'options': [
          'map{string:int}{}',
          'map[string]int{}',
          'Map<string, int>{}',
  ...
  {
    'order': 3,
    'slug': 'go-condicionais',
    'title': 'Condicionais',
    'chapter': 'Capítulo 2: Controle de Fluxo',
    'theory': 'Go usa `if` sem parênteses ao redor da condição.\n\n## Como funciona\n\n```go\nif idade >= 18 {\n```\n\n## No exercício\n\nEscreva o cabeçalho do `if` que verifica se a variável `x` é maior que zero.',
    ...
    'quiz': [
      {
        'question': 'Go usa parênteses na condição do if?',
        'options': [
          'Sim, obrigatórios',
          'Não, são proibidos/desnecessários',
          'Opcional',
          'Só com else',
        ],
        'correct': 1,
      ...
      {
        'question': 'Go tem switch? Como ele difere do C++?',
        'options': [
          'Não tem switch',
          'Tem switch; não precisa de break entre cases',
          'Tem switch igual ao C++',
          'Só suporta strings no switch',
        ],
        'correct': 1,
      },
    ],
  },
  {
    ...
    'title': 'Loop For',
    'chapter': 'Capítulo 2: Controle de Fluxo',
    'theory': 'Go tem apenas `for` como estrutura de repetição (não existe `while`).\n\n## Como funciona\n\n```go\nfor i := 10; i > 0; i-- {\n```\n\n## No exercício\n\nEscreva o cabeçalho do `for` que conta de 0 até 4 (i < 5).',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'for i := 0; i < 5; i++ {',
      },
    ],
    'quiz': [
      {
        'question': 'Go tem loop while?',
        'options': [
          'Sim, com a palavra while',
          'Não; for sozinho age como while',
          'Sim, com a palavra loop',
          'Não, só for...range',
        ],
        'correct': 1,
      },
      {
        'question': 'Como percorrer uma slice em Go?',
        'options': [
          'for item in slice',
          'foreach(slice)',
          'for i, v := range slice',
  ...
  },
  {
    ...
    'slug': 'go-struct',
    'title': 'Struct',
    'chapter': 'Capítulo 4: Tipos e Interfaces',
    'theory': '`struct` em Go agrupa campos relacionados, declarada com `type NomeStruct struct { ... }`.\n\n## Como funciona\n\n```go\ntype Produto struct { Nome string; Preco float64 }\n```\n\n## No exercício\n\nEscreva a struct `Pessoa` com os campos `Nome string` e `Idade int` em uma linha.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'type Pessoa struct { Nome string; Idade int }',
      },
    ],
    'quiz': [
      {
        'question': 'Como definir uma struct em Go?',
        'options': [
          'struct Pessoa { }',
          'class Pessoa { }',
          'type Pessoa struct { }',
      ...
      {
        'question': 'Structs em Go podem ter métodos?',
        'options': [
          'Não, apenas dados',
          'Sim, definidos com receiver',
  ...
  {
    'order': 9,
    'slug': 'go-metodos-struct',
    'title': 'Métodos em Struct',
    'chapter': 'Capítulo 4: Tipos e Interfaces',
    'theory': 'Go adiciona métodos a structs com um *receiver*.\n\n## Como funciona\n\n```go\nfunc (p Produto) Descricao() string {\n```\n\n## No exercício\n\nEscreva a assinatura do método `saudar` que pertence a `Pessoa` e retorna `string`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'func (p Pessoa) saudar() string {',
      },
    ],
    'quiz': [
      {
        'question': 'O que é um receiver em Go?',
        'options': [
          'Um canal de comunicação',
          'O tipo ao qual o método pertence',
          'Um retorno especial',
      ...
      {
        'question': 'A sintaxe de um método com receiver em Go é:',
        'options': [
          'func Pessoa.saudar() {}',
          'func (p Pessoa) saudar() {}',
          'method Pessoa.saudar() {}',
          'func saudar(Pessoa) {}',
        ],
        'correct': 1,
      },
  ...
  },
  {
    ...
    'title': 'Interface',
    'chapter': 'Capítulo 4: Tipos e Interfaces',
    'theory': 'Interfaces em Go definem comportamentos — qualquer struct que implemente os métodos satisfaz a interface automaticamente.\n\n## Como funciona\n\n```go\ntype Forma interface { Area() float64 }\n```\n\n## No exercício\n\nEscreva a interface `Animal` com o método `Falar() string` em uma linha.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'type Animal interface { Falar() string }',
      },
    ],
    'quiz': [
      {
        'question': 'Como uma struct satisfaz uma interface em Go?',
        'options': [
          'Declarando implements explicitamente',
          'Implementando todos os métodos da interface',
          'Herdando a interface',
          'Registrando no compilador',
        ],
        'correct': 1,
      },
      {
        'question': 'Interfaces em Go são:',
        'options': [
          'Explícitas (declara-se implements)',
          'Implícitas (automático ao implementar os métodos)',
          'Opcionais',
          'Apenas para classes',
        ],
        'correct': 1,
...
"ai-prompts": [
  {
    'order': 1,
    'slug': 'prompt-comando-direto',
    'title': 'Comando Direto',
    'chapter': 'Capítulo 1: Fundamentos de Prompting',
    'theory': 'Prompts vagos geram respostas vagas — sempre especifique quantidade, tema e idioma.\n\n## Como funciona\n\n```text\nListe 5 ideias de nomes para um aplicativo de finanças, em português.\n```\n\n## No exercício\n\nSeu prompt deve conter a frase: `3 exemplos de Python para ciência de dados`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': '3 exemplos de Python para ciência de dados',
      },
    ],
    'quiz': [
      {
        'question': 'Por que prompts vagos geram respostas ruins?',
        'options': [
          'A IA é mais lenta com prompts curtos',
          'Falta de contexto e especificidade leva a respostas genéricas',
          'Prompts curtos consomem mais tokens',
          'A IA prefere prompts formais',
        ],
        'correct': 1,
      },
      {
        'question': 'O que especificar em um bom prompt?',
        'options': [
          'Apenas o tema',
          'Quantidade, tema, idioma e formato',
          'Só a pergunta principal',
          'O nome da IA',
        ],
        'correct': 1,
      },
  ...
  },
  {
    ...
    'chapter': 'Capítulo 1: Fundamentos de Prompting',
    'theory': "Atribuir uma persona muda como a IA responde — 'Aja como um professor de matemática' gera respostas mais didáticas.\n\n## Como funciona\n\n```text\nAja como um nutricionista. Sugira um café da manhã saudável.\n```\n\n## No exercício\n\nSeu prompt deve começar com `Aja como`.",
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'Aja como um tutor de programação',
      },
    ],
    'quiz': [
      {
        'question': 'Por que atribuir uma persona melhora as respostas da IA?',
        'options': [
          'A IA fica mais rápida',
          'Orienta o estilo, tom e nível de profundidade da resposta',
          'Economiza tokens',
          'É requisito técnico das APIs',
        ],
        'correct': 1,
      },
      {
        'question': 'Qual frase geralmente inicia um prompt com persona?',
        'options': [
          "'Responda como se fosse'",
          "'Aja como [persona].'",
          "'Simule [persona].'",
          'Qualquer das anteriores funciona igualmente',
        ],
        'correct': 3,
      },
    ],
  ...
  {
    'order': 3,
    'slug': 'prompt-cadeia-pensamento',
    'title': 'Cadeia de Pensamento',
    'chapter': 'Capítulo 2: Técnicas Avançadas',
    'theory': 'Chain-of-thought pede que a IA raciocine passo a passo, melhorando problemas complexos.\n\n## Como funciona\n\n```text\nQuanto é 124 ÷ 4? Pense passo a passo antes de responder.\n```\n\n## No exercício\n\nEscreva um prompt perguntando quanto é 17 × 23 com essa instrução.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'passo a passo antes de responder',
      },
    ],
    'quiz': [
      {
        'question': 'O que chain-of-thought prompting solicita da IA?',
        'options': [
      ...
      {
        'question': 'Chain-of-thought é mais útil para:',
        'options': [
          'Criar imagens',
          'Traduzir textos',
          'Problemas matemáticos e raciocínio complexo',
          'Formatar documentos',
        ],
        'correct': 2,
      },
  ...
  },
  {
    ...
    'chapter': 'Capítulo 2: Técnicas Avançadas',
    'theory': 'Few-shot prompting ensina a IA com exemplos antes de pedir a tarefa.\n\n## Como funciona\n\n```text\nAnimal: Cachorro → Som: Au au. Animal: Gato → Som: Miau. Animal: Vaca → Som:\n```\n\n## No exercício\n\nSeu prompt deve conter `Produto: Café → Slogan:`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'Produto: Café → Slogan:',
      },
    ],
    'quiz': [
      {
        'question': 'O que few-shot prompting significa?',
        'options': [
          'Usar poucas palavras',
          'Fornecer exemplos antes de pedir a tarefa',
      ...
      {
        'question': 'Zero-shot prompting é quando:',
        'options': [
          'O modelo não funciona',
          'A tarefa é pedida sem nenhum exemplo',
          'São dados zero parâmetros',
          'A resposta tem zero erros',
        ],
        'correct': 1,
      },
    ],
  ...
  {
    'order': 5,
    'slug': 'prompt-formato-saida',
    'title': 'Formato de Saída',
    'chapter': 'Capítulo 2: Técnicas Avançadas',
    'theory': 'Especificar o formato de saída torna respostas mais úteis e processáveis.\n\n## Como funciona\n\n```text\nListe 3 capitais europeias em formato JSON, com os campos nome e pais.\n```\n\n## No exercício\n\nSeu prompt deve conter `formato JSON`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'formato JSON',
      },
    ],
    'quiz': [
      {
        'question': 'Por que especificar formato de saída no prompt?',
        'options': [
          'A IA responde mais rápido',
          'A resposta fica mais processável e previsível',
          'Economiza memória',
          'É obrigatório na API',
        ],
        'correct': 1,
      },
      {
        'question': 'Qual dos seguintes especifica formato de saída?',
        'options': [
          "'Responda brevemente'",
          "'Responda em JSON com campos nome e idade'",
  ...
  },
  {
    ...
    'theory': 'Prompts para código devem informar linguagem, nome da função, parâmetros, retorno e extras.\n\n## Como funciona\n\n```text\nEscreva uma função JavaScript chamada somar_lista que receba um array e retorne a soma.\n```\n\n## No exercício\n\nSeu prompt deve conter `calcular_media`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'calcular_media',
      },
    ],
    'quiz': [
      {
        'question': 'O que incluir em um prompt para gerar código?',
        'options': [
          'Apenas o nome da função',
          'Linguagem, nome, parâmetros, retorno e observações',
          'Só a linguagem de programação',
          'Apenas o comportamento esperado',
        ],
        'correct': 1,
      },
      {
        'question': 'Por que pedir docstring no prompt de código?',
        'options': [
          'Ocupa mais espaço',
          'Documenta o código gerado para fácil entendimento',
          'É obrigatório pelo Python',
  ...
  {
    'order': 7,
    'slug': 'prompt-restricoes',
    'title': 'Restrições no Prompt',
    'chapter': 'Capítulo 3: Prompts para Desenvolvimento',
    'theory': 'Restrições tornam respostas mais precisas e adequadas ao contexto.\n\n## Como funciona\n\n```text\nExplique o que é blockchain em no máximo 2 frases, sem jargões técnicos.\n```\n\n## No exercício\n\nSeu prompt deve conter `no máximo 3 frases`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'no máximo 3 frases',
      },
    ],
    'quiz': [
      {
        'question': 'Por que adicionar restrições ao prompt?',
        'options': [
          'Para testar a IA',
          'Para tornar a resposta mais adequada ao contexto e audiência',
      ...
      {
        'question': 'Qual é um exemplo de restrição de prompt?',
        'options': [
          "'Responda em português'",
          "'Sem jargões técnicos, máximo 3 frases, para leigos'",
          "'Use emojis'",
          'Todas acima são restrições',
        ],
        'correct': 3,
      },
    ],
  ...
  {
    'order': 8,
    'slug': 'prompt-refinamento',
    'title': 'Refinamento Iterativo',
    'chapter': 'Capítulo 3: Prompts para Desenvolvimento',
    'theory': 'Raramente o primeiro prompt gera o resultado ideal — o ciclo prompt→resultado→ajuste é a habilidade central do prompting.\n\n## Como funciona\n\n```text\n[Versão 1] Liste 3 dicas de produtividade.\n[Resultado] Dicas genéricas.\n[Ajuste] Liste 3 dicas de produtividade para desenvolvedores remotos, em formato de lista de ações, sem clichês.\n```\n\n## No exercício\n\nSeu prompt deve conter `mais específico`.',
    'instruction_pt': 'O ciclo prompt→resultado→ajuste é a habilidade central do prompting. Dado o prompt vago: `"Explique inteligência artificial."`, escreva um prompt melhorado que peça à IA para deixar a explicação **mais específica**. Seu prompt deve conter `mais específico`.\n\nVeja um exemplo de refinamento:\n\n```text\n[Original] Liste 3 dicas de produtividade.\n[Refinado] Liste 3 dicas de produtividade para devs remotos, em formato de checklist.\n```',
    'instruction_en': 'The prompt→result→adjust cycle is the core skill of prompting. Given the vague prompt: `"Explain artificial intelligence."`, write an improved prompt asking the AI to make the explanation **more specific**. Must contain `mais específico`.',
    'instruction_es': 'El ciclo prompt→resultado→ajuste es la habilidad central. Dado el prompt vago `"Explique inteligência artificial."`, escribe un prompt mejorado que contenga `mais específico`.',
    'starter_code': '// Melhore o prompt: "Explique inteligência artificial."\n// O novo prompt deve conter: mais específico\n// escreva aqui\n',
    'hint': 'Exemplo: Explique inteligência artificial de forma mais específica, focando em como ela é usada em saúde, em 2 parágrafos.',
    'hints': [
      'Seu prompt deve conter `mais específico` — instrua a IA a detalhar melhor o tema.',
      'Inclua: para quem, contexto de uso, formato desejado — isso torna o refinamento concreto.',
      'Exemplo: `Explique IA de forma mais específica, para médicos, focando em diagnóstico por imagem, em 3 tópicos.`',
    ],
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'mais específico',
      },
    ],
    'quiz': [
      {
        'question': 'Por que o refinamento iterativo é importante no prompting?',
        'options': [
          'Para economizar tokens',
          'Porque raramente o primeiro prompt gera o resultado ideal',
          'Para evitar que a IA responda errado',
          'Para diminuir o tamanho da resposta',
        ],
        'correct': 1,
      },
      {
        'question': 'Qual é o ciclo do refinamento iterativo?',
        'options': [
          'Escrever → enviar → apagar',
          'Prompt → resultado → ajuste',
          'Persona → contexto → restrição',
  ...
  {
    'order': 9,
    'slug': 'prompt-projeto-final',
    'title': 'Projeto Final: Prompt Completo',
    'chapter': 'Capítulo 4: Projeto Final',
    'theory': 'Um prompt profissional combina persona + contexto + tarefa + formato + restrições.\n\n## Como funciona\n\n```text\nAja como um mentor de carreira. Analise o perfil de uma designer júnior e dê 3 dicas em lista numerada.\n```\n\n## No exercício\n\nSeu prompt deve conter `coach de carreira`.',
    ...
    'tests': [
      {
        'stdin': '',
        'expected_stdout': 'coach de carreira',
      },
    ],
    'quiz': [
      {
        'question': 'Um prompt profissional completo combina:',
        'options': [
          'Apenas persona e tarefa',
          'Persona + contexto + tarefa + formato + restrições',
          'Só tarefa e formato',
          'Contexto e tarefa apenas',
        ],
        'correct': 1,
      },
      {
        'question': 'Qual elemento do prompt define o estilo da resposta?',
        'options': [
          'Contexto',
          'Tarefa',
          'Persona',
      ...
      {
        'question': "Qual elemento define limitações como 'máximo 3 itens'?",
        'options': [
          'Persona',
          'Contexto',
          'Formato',
