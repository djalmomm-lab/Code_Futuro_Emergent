...
"python-zero": [
  ...
  },
  {
    ...
    "theory": "## Tomando decisões com if/else\n\nO `if` permite que seu programa tome **decisões** baseadas em condições:\n\n```python\nidade = 18\n\nif idade >= 18:\n    print(\"Adulto\")\nelse:\n    print(\"Menor de idade\")\n# Saída: Adulto\n```\n\n### Como funciona?\n1. O Python avalia a condição (`idade >= 18`)\n2. Se for **verdadeira** → executa o bloco do `if`\n3. Se for **falsa** → executa o bloco do `else`\n\n### ⚠️ Recuo (indentação) é obrigatório!\n**Indentação** é o nome técnico para os espaços no início de uma linha — é como se fosse um \"recuo\" ou \"margem\" do texto. Em Python, o código dentro do `if` e `else` precisa ter **4 espaços** (ou 1 tab) no início de cada linha.\n\n```python\nif idade >= 18:\n    print(\"Adulto\")   # ← 4 espaços antes = está DENTRO do if ✅\nprint(\"Fim\")          # ← sem espaço = está FORA do if ✅\n\nif idade >= 18:\nprint(\"Adulto\")       # ← sem espaço = ERRO! ❌\n```\n\nPense assim: o recuo mostra para o Python onde começa e termina cada bloco de código.",
