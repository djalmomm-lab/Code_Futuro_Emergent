...
"python-zero": [
  ...
  },
  {
    ...
    "theory": "## Tomando decisões com if/else\n\nO `if` permite que seu programa tome **decisões** baseadas em condições:\n\n```python\nidade = 18\n\nif idade >= 18:\n    print(\"Adulto\")\nelse:\n    print(\"Menor de idade\")\n# Saída: Adulto\n```\n\n### Como funciona?\n1. O Python avalia a condição (`idade >= 18`)\n2. Se for **verdadeira** → executa o bloco do `if`\n3. Se for **falsa** → executa o bloco do `else`\n\n---\n\n### 🔢 Como escrever os símbolos matemáticos no teclado\n\nNo Python **não usamos** os símbolos matemáticos do caderno (≥, ≤, ≠). Usamos sequências de teclas:\n\n| O que significa | Símbolo matemático | Como digitar no Python |\n|---|---|---|\n| igual a | = | `==` (dois sinais de igual) |\n| diferente de | ≠ | `!=` (exclamação + igual) |\n| maior que | > | `>` |\n| menor que | < | `<` |\n| maior ou igual a | ≥ | `>=` (maior + igual, sem espaço) |\n| menor ou igual a | ≤ | `<=` (menor + igual, sem espaço) |\n\n⚠️ **Atenção:** `>=` e `<=` devem ser escritos **sem espaço** entre os dois caracteres!\n\n```python\n# ✅ Correto\nif idade >= 18:\n\n# ❌ Errado — com espaço entre > e =\nif idade > = 18:\n```\n\n---\n\n### ⚠️ Recuo (indentação) é obrigatório!\n**Indentação** é o nome técnico para os espaços no início de uma linha — é como se fosse um \"recuo\" ou \"margem\" do texto. Em Python, o código dentro do `if` e `else` precisa ter **4 espaços** no início de cada linha.\n\n```python\nif idade >= 18:\n    print(\"Adulto\")   # ← 4 espaços = está DENTRO do if ✅\nprint(\"Fim\")          # ← sem espaço = está FORA do if ✅\n```",
