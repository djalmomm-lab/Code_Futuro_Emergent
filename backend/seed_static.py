...
"python-zero": [
  ...
  },
  {
    ...
    "instruction_pt": "O código ao lado tem dois `___` para você completar:\n\n```python\ndef saudacao(___):\n    return ___\n\nprint(saudacao(\"Ana\"))\n```\n\n- **1º `___`** → o nome do parâmetro que a função recebe (ex: `nome`)\n- **2º `___`** → a mensagem que a função retorna usando f-string\n\nQuando correto, `print(saudacao(\"Ana\"))` deve imprimir exatamente:\n```\nOlá, Ana!\n```",
