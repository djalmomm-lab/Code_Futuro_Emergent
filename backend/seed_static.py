...
"python-zero": [
  ...
  },
  {
    ...
    "title": "Comparadores",
    "chapter": "Capítulo 2: Controle de Fluxo",
    "theory": "## Comparadores em Python\n\nComparadores comparam dois valores e retornam `True` (verdadeiro) ou `False` (falso):\n\n```python\nprint(10 > 5)   # True  — 10 é maior que 5?\nprint(10 < 5)   # False — 10 é menor que 5?\nprint(10 == 10) # True  — 10 é igual a 10?\nprint(10 != 5)  # True  — 10 é diferente de 5?\nprint(10 >= 10) # True  — 10 é maior ou igual a 10?\nprint(10 <= 9)  # False — 10 é menor ou igual a 9?\n```\n\n---\n\n### 🔢 Como digitar cada símbolo no teclado\n\nNo Python **não usamos** os símbolos do caderno (≥, ≤, ≠). Veja como digitar cada um:\n\n| Símbolo matemático | Como digitar | Significado |\n|---|---|---|\n| = | `==` | igual a (dois = seguidos) |\n| ≠ | `!=` | diferente de (! seguido de =) |\n| > | `>` | maior que |\n| < | `<` | menor que |\n| ≥ | `>=` | maior ou igual (> seguido de =, sem espaço) |\n| ≤ | `<=` | menor ou igual (< seguido de =, sem espaço) |\n\n⚠️ **Importante:** `>=` e `<=` devem ser escritos **colados**, sem espaço!\n\n```python\nif nota >= 7:   # ✅ correto\nif nota > = 7:  # ❌ errado — espaço entre > e =\n```\n\n---\n\n### ⚠️ = vs ==\n- `=` **atribui** um valor a uma variável: `idade = 15`\n- `==` **compara** dois valores: `idade == 15`\n\n```python\nidade = 15        # ← guarda o valor 15 na variável\nprint(idade == 15) # ← pergunta: idade é igual a 15? → True\nprint(idade == 16) # ← pergunta: idade é igual a 16? → False\n```",
